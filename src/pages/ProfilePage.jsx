...
export default function ProfilePage() {
  ...
  const { connect, sendDraw, sendMessage, onRemoteDraw, sendProductDrop } = useSocket();
  ...
  const ProdCard = ({p}) => {
    ...
    return (
      ...
      >
        ...
        <div style={{display:'flex',gap:4,padding:'0 7px 8px'}}>
          <button onClick={handleCanvas} style={{flex:1,background:'#f4f0ff',color:'#7c3aed',border:'1.5px solid #ede9fe',borderRadius:8,padding:'6px 2px',fontSize:'.68rem',fontWeight:700,cursor:'pointer',minHeight:36,fontFamily:'inherit'}}>🎨</button>
          <button onClick={e=>{e.stopPropagation();safeOpen(p.url);}} style={{flex:2,background:'linear-gradient(90deg,#7c3aed,#5b21b6)',color:'#fff',border:'none',borderRadius:8,padding:'6px 4px',fontSize:'.68rem',fontWeight:700,cursor:'pointer',minHeight:36,fontFamily:'inherit'}}>Buy →</button>
          <button onClick={e=>{e.stopPropagation();if(isWishlisted){wishlistRm(p.id);showToast('♡ Removed');}else{addToWishlist(p);}}} style={{flex:1,background:isWishlisted?'#fee2e2':'#f4f0ff',color:isWishlisted?'#b91c1c':'#7c3aed',border:`1.5px solid ${isWishlisted?'#fca5a5':'#ede9fe'}`,borderRadius:8,padding:'6px 2px',fontSize:'.75rem',fontWeight:700,cursor:'pointer',minHeight:36,fontFamily:'inherit'}}>{isWishlisted?'♥':'♡'}</button>
        </div>
      </div>
    );
  };
