...
CSS = (
    # Layout
    ".pw{max-width:960px;margin:0 auto;padding-bottom:2rem}"
    # Cover
    ".cover{position:relative;height:240px;background:linear-gradient(135deg,#7c3aed 0%,#4c1d95 50%,#1e1b4b 100%);border-radius:16px 16px 0 0;overflow:hidden}"
    ".cover-edit{position:absolute;bottom:12px;right:12px;background:rgba(0,0,0,.55);color:#fff;border:none;border-radius:8px;padding:6px 14px;font-size:.78rem;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:6px}"
    # Profile card
    ".profile-card{background:#fff;border-radius:0 0 16px 16px;padding:0 2rem 1.5rem;box-shadow:0 4px 24px rgba(124,58,237,.1);margin-bottom:1.25rem}"
    ".avatar-wrap{position:relative;display:inline-block;margin-top:-52px;margin-bottom:.6rem}"
    ".av{width:100px;height:100px;border-radius:50%;background:linear-gradient(135deg,#7c3aed,#5b21b6);color:#fff;font-size:2.2rem;font-weight:800;display:flex;align-items:center;justify-content:center;border:4px solid #fff;box-shadow:0 2px 16px rgba(124,58,237,.35)}"
    ".av-cam{position:absolute;bottom:4px;right:4px;width:30px;height:30px;border-radius:50%;background:#7c3aed;color:#fff;border:2px solid #fff;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:.7rem;font-weight:800}"
    ".live-ring{position:absolute;inset:-4px;border-radius:50%;border:3px solid #ef4444;animation:lp 1.5s infinite}"
    "@keyframes lp{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.5;transform:scale(1.06)}}"
    ".profile-top{display:flex;align-items:flex-end;justify-content:space-between;flex-wrap:wrap;gap:1rem;padding-top:.5rem}"
    ".profile-info{flex:1;min-width:200px}"
    ".profile-name{font-size:1.45rem;font-weight:800;color:#1a0a3e;display:flex;align-items:center;gap:.5rem;flex-wrap:wrap}"
    ".verified{color:#7c3aed;font-size:1.1rem}"
    ".live-badge{background:#ef4444;color:#fff;font-size:.62rem;font-weight:800;padding:2px 9px;border-radius:20px;letter-spacing:.06em;animation:lp 1.5s infinite}"
    ".handle{font-size:.82rem;color:#6b7280;margin:.2rem 0 .75rem}"
    ".pstats{display:flex;gap:1.5rem;flex-wrap:wrap;margin-bottom:.85rem}"
    ".pstat{text-align:center;cursor:pointer}"
    ".pstat:hover .pstat-num{color:#7c3aed}"
    ".pstat-num{font-size:1.15rem;font-weight:800;color:#1a0a3e;transition:.15s}"
    ".pstat-lbl{font-size:.7rem;color:#9ca3af;text-transform:uppercase;letter-spacing:.04em}"
    ".pactions{display:flex;gap:.6rem;flex-wrap:wrap;align-items:center}"
    ".btn-pri{background:linear-gradient(90deg,#7c3aed,#5b21b6);color:#fff;border:none;border-radius:10px;padding:.55rem 1.1rem;font-size:.82rem;font-weight:700;cursor:pointer;display:inline-flex;align-items:center;gap:.4rem;transition:.15s}"
    ".btn-pri:hover{opacity:.9}"
    ".btn-sec{background:#f4f0ff;color:#7c3aed;border:1.5px solid #ede9fe;border-radius:10px;padding:.5rem 1rem;font-size:.82rem;font-weight:700;cursor:pointer;display:inline-flex;align-items:center;gap:.4rem;transition:.15s}"
    ".btn-sec:hover{background:#ede9fe}"
    ...
    ".btn-end{background:#f4f0ff;color:#ef4444;border:1.5px solid #fca5a5;border-radius:10px;padding:.55rem 1.1rem;font-size:.82rem;font-weight:700;cursor:pointer;display:inline-flex;align-items:center;gap:.4rem}"
    # Tab bar with scroll arrows
    ".tab-wrap{position:relative;margin-bottom:1.25rem}"
    ".tab-arrow{position:absolute;top:50%;transform:translateY(-50%);z-index:2;width:32px;height:32px;border-radius:50%;background:#fff;border:1.5px solid #ede9fe;color:#7c3aed;font-size:.85rem;font-weight:800;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(124,58,237,.15);transition:.15s}"
    ".tab-arrow:hover{background:#f4f0ff}"
    ".tab-arrow-l{left:0}"
    ".tab-arrow-r{right:0}"
    ".ptabs{display:flex;gap:0;background:#fff;border-radius:12px;padding:.4rem;box-shadow:0 2px 12px rgba(124,58,237,.08);overflow-x:auto;scroll-behavior:smooth;scrollbar-width:none;margin:0 40px}"
    ".ptabs::-webkit-scrollbar{display:none}"
    ".ptab{display:flex;align-items:center;gap:.35rem;padding:.55rem .85rem;border-radius:8px;border:none;background:transparent;color:#6b7280;font-family:inherit;font-size:.82rem;font-weight:600;cursor:pointer;white-space:nowrap;transition:.15s;flex-shrink:0}"
    ...
    ".feed-av{width:40px;height:40px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:.82rem;font-weight:800;flex-shrink:0;color:#fff;background:linear-gradient(135deg,#7c3aed,#5b21b6)}"
    ".feed-text{font-size:.85rem;color:#374151;line-height:1.5}"
    ".feed-time{font-size:.72rem;color:#9ca3af;margin-top:.2rem}"
    # Whiteboard tab
    ".wb-canvas{width:100%;height:340px;background:#fff;border-radius:12px;border:1.5px solid #ede9fe;cursor:crosshair;display:block}"
    ".wb-tools{display:flex;gap:.6rem;align-items:center;flex-wrap:wrap;margin-bottom:.75rem}"
    ".wb-color{width:28px;height:28px;border-radius:50%;border:2px solid transparent;cursor:pointer;transition:.15s}"
    ".wb-color.active,.wb-color:hover{border-color:#7c3aed;transform:scale(1.15)}"
    ".wb-btn{background:#f4f0ff;color:#7c3aed;border:1.5px solid #ede9fe;border-radius:8px;padding:5px 12px;font-size:.78rem;font-weight:700;cursor:pointer}"
    # Messages tab
    ".msg-list{height:300px;overflow-y:auto;padding:.5rem 0;margin-bottom:.75rem;border-bottom:1px solid #f3f4f6}"
    ".msg-row{display:flex;gap:.6rem;margin-bottom:.75rem;align-items:flex-end}"
    ".msg-row.me{flex-direction:row-reverse}"
    ".msg-bubble{max-width:70%;padding:.6rem .9rem;border-radius:14px;font-size:.85rem;line-height:1.5}"
    ".msg-row:not(.me) .msg-bubble{background:#f4f0ff;color:#1a0a3e;border-bottom-left-radius:4px}"
    ".msg-row.me .msg-bubble{background:linear-gradient(90deg,#7c3aed,#5b21b6);color:#fff;border-bottom-right-radius:4px}"
    ".msg-av{width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg,#7c3aed,#5b21b6);color:#fff;font-size:.72rem;font-weight:800;display:flex;align-items:center;justify-content:center;flex-shrink:0}"
    ".msg-input-row{display:flex;gap:.6rem}"
    # Live tab
    ".live-card{background:linear-gradient(135deg,#1e1b4b,#4c1d95);border-radius:14px;padding:1.5rem;color:#fff;text-align:center;margin-bottom:1rem}"
    ".live-viewers{font-size:2rem;font-weight:800;color:#fbbf24}"
    ".past-session{display:flex;align-items:center;justify-content:space-between;padding:.85rem;border-radius:12px;border:1.5px solid #f3f4f6;margin-bottom:.6rem}"
    ".ps-name{font-size:.88rem;font-weight:700;color:#1a0a3e}"
    ".ps-meta{font-size:.75rem;color:#9ca3af}"
    # Badges
    ".badge-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:.75rem}"
    ".badge-card{background:#f4f0ff;border-radius:12px;padding:.85rem;text-align:center;border:1.5px solid #ede9fe;transition:.15s}"
    ".badge-card:hover{border-color:#7c3aed}"
    ".badge-icon{font-size:1.6rem;margin-bottom:.35rem}"
    ".badge-name{font-size:.72rem;font-weight:700;color:#7c3aed}"
    ".badge-desc{font-size:.65rem;color:#9ca3af;margin-top:.15rem}"
    # Stats bars
    ".stat-bar{display:flex;align-items:center;gap:.75rem;margin-bottom:.65rem}"
    ...
    ".ref-link{font-size:.82rem;font-family:monospace;background:#fff;border:1.5px solid #ede9fe;border-radius:8px;padding:.5rem .9rem;color:#7c3aed;font-weight:700;display:inline-block;margin:.5rem 0}"
    # Rooms
    ...
    ".rb{font-size:.65rem;font-weight:700;padding:2px 8px;border-radius:20px}"
    ...
    ".order-img{width:52px;height:52px;border-radius:8px;background:#f4f0ff;display:flex;align-items:center;justify-content:center;font-size:1.3rem;flex-shrink:0}"
    ...
    "@media(max-width:768px){.pstats{gap:1rem}.pactions{gap:.4rem}.ptabs{margin:0 36px}}"
)

jsx = r"""import { useState, useRef, useEffect } from 'react';
...
# Verify all tabs and key features
checks = [
    ("const CSS = `",           "CSS backtick open"),
    ("`;",                       "CSS backtick close"),
    ("tab==='feed'",             "Activity tab"),
    ("tab==='whiteboard'",       "Whiteboard tab"),
    ("tab==='profile'",          "Profile tab"),
    ("tab==='friends'",          "Friends tab"),
    ("tab==='messages'",         "Messages tab"),
    ...
    ("tab==='stats'",            "Stats tab"),
    ("tab==='referral'",         "Referral tab"),
    ("tab==='notifs'",           "Notifications tab"),
    ("tab==='security'",         "Security tab"),
    ("tab==='data'",             "Your Data tab"),
    ("scrollTabs",               "Tab scroll arrows"),
    ("wbStart",                  "Whiteboard drawing"),
    ("msg-bubble",               "Messages UI"),
    ("PAST_SESSIONS",            "Live sessions history"),
]
...
print()
if all_ok:
    print('ALL CHECKS PASSED — ready to build and push')
else:
    print('SOME CHECKS FAILED — do not push')
