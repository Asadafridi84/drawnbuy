...
const CHAT_INIT = [
  ...
  {id:2,av:'L',name:'Lars',text:'Added this from your wishlist!',time:'14:21',col:'#0ea5e9',me:false,
   prod:{name:'IKEA SÖDERHAMN',price:'4 295 kr',img:'https://images.unsplash.com/photo-1555041469-a586c61ea9bc?w=100&q=70'}},
...
function Modal({open,onClose,title,subtitle,children,width=430}){
  ...
  return(
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.5)',zIndex:500,display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(4px)'}}>
      <div style={{background:'#fff',borderRadius:'20px',padding:'1.75rem',maxWidth:``${width}px`,width:'90%',position:'relative',animation:'ppPopIn .25s ease both'}}>
      ...
      <div className="ppgrid">
          ...
          <div className="pplp">
            <div className="pppc">
              ...
              <div style={{padding:'.7rem .9rem'}}>
                {[{v:'all',l:'Everyone',c:``${friends.length} people`},{v:'family',l:'Family only',c:``${friends.filter(f=>f.rel==='Family').length} people`},{v:'friends',l:'Friends only',c:``${friends.filter(f=>f.rel==='Friend').length} people`},{v:'custom',l:'Custom — pick people',c:null}].map(o=>(
        ...
        {tab==='canvas'&&(
          <div className="ppcc">
            <div className="ppcw">
              ...
              {syncNote&&(
                <div className="ppsbar"><div className="ppsdot"/><span>{syncNote} — appears on all your active canvases</span></div>
              )}
              <div className="ppca" id="ppca">
                ...
                {!drawn&&stickers.length===0&&(
                  <div className="ppch">
                    ...
                    <div className="ppcht">Draw your wishlist — or sync products from the main page!</div>
                  </div>
                )}
                {stickers.map(s=>(
                  <div key={s.sid} className="ppsticker" style={{left:``${s.x}px`,top:``${s.y}px`}}
                    onMouseDown={e=>{
        ...
        {tab==='data'&&(
          <div style={{gridColumn:'1/-1',animation:'ppFadeUp .3s ease both'}}>
            <div className="pptc" style={{maxWidth:'500px'}}>
              <div className="pppb"><ILock/> Only you can see this</div>
              <h3>Your Data</h3><p>GDPR — you own and control all your data</p>
