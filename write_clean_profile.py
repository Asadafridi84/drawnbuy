...
""" + "const CSS = `" + r"""
.pw{display:grid;grid-template-columns:260px 1fr;gap:1.5rem;max-width:960px;margin:0 auto;align-items:start}
...
.fc{display:flex;align-items:center;gap:.9rem;padding:.85rem;border-radius:12px;border:1.5px solid #f3f4f6;background:#fafafa;margin-bottom:.6rem}
      ...
      <div className="pw">
        <div className="ps">
          ...
          <div className="pc">
            ...
            {stats.map(({label,value,Icon})=>(<div key={label} className="sr"><span className="si"><Icon/></span><span className="sl">{label}</span><span className="sv">{value||'0'}</span></div>))}
          </div>
          <div className="pc" style={{padding:'.75rem'}}>
            {TABS.map(({id,label,icon:Icon})=>(<button key={id} className={'tb'+(tab===id?' on':'')} onClick={()=>setTab(id)}><span style={{display:'flex',alignItems:'center'}}><Icon/></span>{label}</button>))}
          </div>
        ...
        <div>
          {tab==='profile' && (<div className="pc"><div className="st">Profile Information</div><div className="ss">Update your name and bio.</div><div style={{marginBottom:'1rem'}}><label className="fl">Display Name</label><input className="fi" value={name} onChange={e=>setName(e.target.value)} placeholder="Your name"/></div><div style={{marginBottom:'1rem'}}><label className="fl">Email</label><input className="fi" value={user?.email||''} disabled/></div><div style={{marginBottom:'1.25rem'}}><label className="fl">Bio <span style={{color:'#9ca3af',fontWeight:400}}>{bio.length}/300</span></label><textarea className="fi" value={bio} onChange={e=>setBio(e.target.value.slice(0,300))} rows={4} placeholder="Tell people what you shop for..." style={{resize:'vertical'}}/></div>{saved?<div className="ok"><IS/> Saved!</div>:<button className="bp" onClick={doSave}><IS/> Save Changes</button>}</div>)}
          {tab==='friends' && (<div className="pc"><div className="st">Friends &amp; Family</div><div className="ss">Shop and draw together on shared canvases.</div><div style={{background:'#f4f0ff',borderRadius:'12px',padding:'1rem',marginBottom:'1.25rem',border:'1.5px solid #ede9fe'}}><div style={{display:'grid',gridTemplateColumns:'1fr auto auto',gap:'.6rem',alignItems:'end'}}><div><label className="fl">Email</label><input className="fi" value={iEmail} onChange={e=>setIEmail(e.target.value)} placeholder="friend@example.com" type="email"/></div><div><label className="fl">Relation</label><select className="fi" value={iRel} onChange={e=>setIRel(e.target.value)} style={{width:'120px'}}><option>Friend</option><option>Family</option><option>Partner</option></select></div><button className="bp" onClick={doInvite}><IP/> Invite</button></div>{iSent&&<div className="ok" style={{marginTop:'.6rem'}}>Sent!</div>}</div>{friends.map(f=>(<div key={f.id} className="fc"><div className="fa">{f.av}</div><div style={{flex:1}}><div className="fn">{f.name}</div><div className="fe">{f.email}</div></div><div className={'sd '+(f.status==='online'?'son':f.status==='invited'?'sin':'sof')} title={f.status}/><button onClick={()=>setFriends(p=>p.filter(x=>x.id!==f.id))} style={{background:'none',border:'none',cursor:'pointer',color:'#d1d5db'}}><IT/></button></div>))}{friends.length===0&&<div style={{textAlign:'center',color:'#9ca3af',padding:'2rem'}}>No connections yet.</div>}</div>)}
          {tab==='wishlist' && (<div className="pc"><div className="st">Your Wishlist</div><div style={{textAlign:'center',padding:'3rem',color:'#9ca3af'}}><IW/><div style={{marginTop:'1rem',fontSize:'.88rem'}}>No saved products yet.</div><button className="bp" style={{marginTop:'1rem'}} onClick={()=>navigate('/')}>Browse Products</button></div></div>)}
          {tab==='notifs' && (<div className="pc"><div className="st">Notifications</div>{[{key:'deals',label:'Flash Deals',sub:'New deals'},{key:'collabs',label:'Canvas Invites',sub:'Draw invitations'},{key:'news',label:'Updates',sub:'New features'},{key:'weekly',label:'Weekly Digest',sub:'Trending products'}].map(({key,label,sub})=>(<div key={key} className="nr"><div><div className="nl">{label}</div><div className="ns">{sub}</div></div><label className="tog"><input type="checkbox" checked={notifs[key]} onChange={e=>setNotifs(n=>({...n,[key]:e.target.checked}))}/><span className="tsl"/></label></div>))}<div style={{marginTop:'1rem'}}><button className="bp" onClick={doSave}><IS/> Save</button></div></div>)}
          {tab==='security' && (<div className="pc"><div className="st">Security</div><div style={{padding:'.85rem',background:'#f9fafb',borderRadius:'10px',border:'1px solid #f3f4f6'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><div style={{fontSize:'.9rem',fontWeight:700,color:'#1a0a3e'}}>Password</div><div style={{fontSize:'.78rem',color:'#9ca3af'}}>Last changed: Never</div></div><button className="bg2">Change Password</button></div></div></div>)}
          {tab==='data' && (<div className="pc"><div className="st">Your Data</div><div className="ss">GDPR compliant. Data stored in Sweden.</div><div style={{display:'flex',gap:'1rem'}}><button className="bg2"><ID/> Export</button><button className="bd"><IT/> Delete</button></div></div>)}
        </div>
      </div>
      {delConf&&(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.5)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center'}}><div style={{background:'#fff',borderRadius:'16px',padding:'2rem',maxWidth:'400px',width:'90%'}}><div style={{fontSize:'1.1rem',fontWeight:800,color:'#1a0a3e',marginBottom:'.5rem'}}>Delete Account?</div><div style={{fontSize:'.88rem',color:'#6b7280',marginBottom:'1.5rem'}}>This cannot be undone.</div><div style={{display:'flex',gap:'.75rem'}}><button className="bg2" style={{flex:1,justifyContent:'center'}} onClick={()=>setDelConf(false)}>Cancel</button><button className="bd" style={{flex:1,justifyContent:'center'}} onClick={()=>{logout();navigate('/')}}>Yes, Delete</button></div></div></div>)}
    </div>
  );
}
"""

pathlib.Path('src/pages/ProfilePage.jsx').write_text(jsx, encoding='utf-8')
lines = jsx.count('\n')
print(f'Written: {lines} lines')

# Verify key parts
assert 'const CSS = `' in jsx, 'MISSING: CSS opening backtick'
assert '`;' in jsx, 'MISSING: CSS closing backtick'
assert "className={'tb'+(tab===id?' on':'')}" in jsx, 'MISSING: tab className'
